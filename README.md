...
E2E browser nyata **lintas-browser**: `tests/e2e.js` (Chromium 29/29 & Firefox 29/29) · `tests/a11y.js` axe-core 6/6 · Lighthouse 93/100/100/100 + `tests/audit-browser.js` (recon 142 node, 160/160) · **Visual QA matrix `tests/visual.js` (32/32)**: 7 viewport × 4 layar tanpa overflow horizontal, nol error JS, screenshot di `design/` — onboarding → sesi penuh (MC/numeric/steps) → streak → 4 layar → export `.fgy` + dekripsi roundtrip Node → modal konsep → pabrik → **nol error JS**. (Butuh `playwright-core`; sandbox workspace: `/home/user/e2e-work`.)
