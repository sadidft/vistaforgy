...
(function () {
  ...
  UI.route('settings', function () {
    ...
    var html = '<section class="screen narrow"><h1>Pengaturan</h1>' +
      ...
      '<div class="field"><span>Target harian</span><div class="dd" id="ddGoal"></div></div>' +
      '<div class="field"><span>Track</span><div class="dd" id="ddTrack"></div></div></div>' +
      '<div class="panel">' +
      '<div class="toggle-row"><span>' + ic('zap') + ' Efek suara</span><button class="toggle ' + (s.sound ? 'on' : '') + '" id="setSound"></button></div>' +
      '<div class="toggle-row"><span>Volume</span><input aria-label="Volume suara" type="range" id="setVol" min="0" max="100" value="' + Math.round(s.volume * 100) + '"></div>' +
      ...
      '<div class="panel"><h2 class="ph">Profil</h2>' +
      '<div id="profList"></div>' +
      '<div class="field" style="margin-top:10px"><span>Profil baru (mis. teman yang pinjam device-mu)</span>' +
      '<input id="profName" type="text" maxlength="16" placeholder="nama profil" aria-label="Nama profil baru"></div>' +
      '<button class="btn ghost small" id="profAdd">Tambah profil & pindah</button>' +
      '<p class="muted small">Progress tiap profil terpisah penuh (slot lokal sendiri). Profil aktif otomatis tersimpan ke slotnya saat pindah.</p></div>' +
      '<div class="panel"><h2 class="ph">Pelajari sistemnya</h2><div class="btn-row">' +
      ...
      '<div class="panel danger-zone"><h2 class="ph">Zona Merah</h2><p class="muted small">Menghapus seluruh progress. Aplikasi akan menawarkan export dulu.</p>' +
    ...
    function tgl(id, key, label, cb) {
      var b = UI.$(id);
      function sync() {
        b.classList.toggle('on', save.settings[key]);
        b.setAttribute('aria-pressed', save.settings[key] ? 'true' : 'false');
        b.setAttribute('aria-label', label + ': ' + (save.settings[key] ? 'aktif' : 'nonaktif'));
      }
      sync();
      b.onclick = function () { save.settings[key] = !save.settings[key]; sync(); VF.persist(); if (cb) cb(save.settings[key]); };
    }
    tgl('#setSound', 'sound', 'Efek suara');
    tgl('#setSerious', 'serious', 'Serius Mode', function (on) { var l = VF.KOA.line(save, on ? 'serius-on' : 'serius-off'); if (l) UI.toast(esc(l)); });
    ...
    function renderProfiles() {
      var host = UI.$('#profList');
      if (!host) return;
      var active = STORE.activeProfile();
      var idx = STORE.profileIndex();
      if (!idx.some(function (p) { return p.name === active; })) idx.push({ name: active });
      host.innerHTML = idx.map(function (p) {
        var isA = p.name === active;
        return '<div class="kv"><span>' + esc(p.name) + (isA ? ' <b class="ok">· aktif</b>' : '') + '</span>' +
          '<span class="kv-r">' + (isA ? '' : '<button class="mini-btn prof-sw" data-n="' + esc(p.name) + '">pindah</button>') +
          (isA ? '' : ' <button class="mini-btn prof-del" data-n="' + esc(p.name) + '">hapus</button>') + '</span></div>';
      }).join('');
      UI.$$('.prof-sw', host).forEach(function (b) {
        b.onclick = function () {
          var n = b.getAttribute('data-n');
          var m = UI.modal('<div class="concept"><h2>Pindah ke "' + esc(n) + '"?</h2><p>Progress profil saat ini otomatis disimpan ke slotnya dulu.</p>' +
            '<div class="btn-row"><button class="btn primary" id="psY">Pindah</button><button class="btn ghost" id="psN">Batal</button></div></div>');
          UI.$('#psY', m.box).onclick = function () {
            if (STORE.switchProfile(n)) { m.close(); location.reload(); } // JANGAN persist di sini: switchProfile sudah menulis slot; persist akan menimpa dgn save in-memory basi
            else { UI.toast('Slot profil kosong/rusak.', 'warn'); m.close(); }
          };
          UI.$('#psN', m.box).onclick = m.close;
        };
      });
      UI.$$('.prof-del', host).forEach(function (b) {
        b.onclick = function () { STORE.deleteProfile(b.getAttribute('data-n')); renderProfiles(); };
      });
    }
    renderProfiles();
    UI.$('#profAdd').onclick = function () {
      var n = UI.$('#profName').value.trim();
      if (!n) { UI.toast('Isi nama profil dulu.', 'warn'); return; }
      VF.persist();
      var fresh = STORE.newSave();
      fresh.settings = JSON.parse(JSON.stringify(VF.save.settings));
      if (STORE.createProfile(n, fresh)) {
        UI.toast('Profil "' + esc(n) + '" dibuat & aktif.', 'ok');
        setTimeout(function () { location.reload(); }, 600);
      } else UI.toast('Nama sudah dipakai.', 'warn');
    };
